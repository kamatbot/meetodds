...
pub fn get_default_recordings_folder() -> PathBuf {
    ...
    {
        // Windows: %USERPROFILE%\Music\meetodds-recordings
        if let Some(music_dir) = dirs::audio_dir() {
            music_dir.join("meetodds-recordings")
        } else {
            // Fallback to Documents if Music folder is not available
            dirs::document_dir()
                .unwrap_or_else(|| PathBuf::from("."))
                .join("meetodds-recordings")
        }
    }

    #[cfg(target_os = "macos")]
    {
        // macOS: ~/Movies/meetodds-recordings
        if let Some(movies_dir) = dirs::video_dir() {
            movies_dir.join("meetodds-recordings")
        } else {
    ...
    {
        // Linux/Others: ~/Documents/meetodds-recordings
        dirs::document_dir()
            .unwrap_or_else(|| PathBuf::from("."))
            .join("meetodds-recordings")
    }
